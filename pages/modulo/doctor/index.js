...
export default function Doctor({ recordset }) {
  ...
  const activardesactivar = (usuario, activoinactivo) => {
    const peticion = fetch(
      `http://${process.env.NEXT_PUBLIC_SERVER}:3000/api/doctor?usuario=${usuario}&activoinactivo=${
        activoinactivo ? "1" : "0"
      }`,
      { method: "DELETE" }
    );
  ...
  const enviarFormularioDoctor = () => {
    const peticion = fetch(
      `http://${process.env.NEXT_PUBLIC_SERVER}:3000/api/doctor?usuario=${nombreRef.current.value}&apellido=${apellidoRef.current.value}&colegiado=${colegiadoRef.current.value}`,
      { method: "POST" }
    );
    peticion
      .then((response) => response.json())
  ...
  useEffect(() => {
    const peticion = fetch(`http://${process.env.NEXT_PUBLIC_SERVER}:3000/api/especialidades`, {
      method: "GET",
    });
    peticion
      .then((response) => response.json())
